...
> **Side panel implémenté ([ADR-008](adr/0008-side-panel.md),
> [ADR-009](adr/0009-replication.md))** : ce document décrit désormais
> l'architecture où le side panel est la surface principale de saisie et
> de lecture — le site n'est plus qu'une destination d'écriture, atteinte
> par écrasement total sur un ciblage explicite persistant par site.
> Remplace l'architecture précédente (fail-closed, mais injection directe
> dans le champ du site) après trois échecs successifs de détection/
> injection sur les vrais sites — voir `bugs.md` et ADR-008, Contexte.
> Détail technique complet dans [docs/recherche/reco.md](recherche/reco.md)
> (R-01 à R-63) pour l'historique antérieur au pivot.
...
- **Fail-closed ([ADR-007](adr/0007-fail-closed.md))** : le tag `[TYP:CODE]`
  est la source de vérité, jamais le vrai nom écrit dans un champ qui part
  au réseau. Transposé au panneau ([ADR-008](adr/0008-side-panel.md)) : le
  vrai nom n'est décoré qu'en overlay, dans le panneau, jamais dans le DOM
  du site.
...
│   ├── editor-handle/        # façade de saisie (R-08 → R-18) — le champ du panneau est toujours un <textarea>
│   │   └── textarea-handle.js       #   mesure par miroir (R-19 → R-24) ; contenteditable-handle.js retiré (plus de champ contenteditable à gérer, seul le site l'était, plus jamais atteint via EditorHandle)
│   ├── display.js            # calque de décoration (R-25 → R-46) : racine shadow DOM fermée, légende, infobulle, soulignement — chargé par le panneau, pas par le site
│   ├── mention-menu.js       # menu déclenché par "&" (M-03/M-04) : insère le tag via EditorHandle — chargé par le panneau
│   └── pseudonyme.js         # génération/résolution de code + regex de tag partagée (M-10) — chargé par le panneau
...
Point notable : `editor-handle/`, `display.js`, `mention-menu.js` et
`pseudonyme.js` sont **exactement les mêmes fichiers** qu'avant le pivot
side panel, non modifiés — voir [ADR-008](adr/0008-side-panel.md),
Conséquences. Ils n'ont jamais eu besoin de savoir sur quelle page ils
s'exécutaient ; seul le point d'entrée qui les charge change (`sidepanel/`
au lieu du `content_scripts` du site).
...
| `content/mention-menu.js` | Détecte `&`, affiche le menu, insère le tag `[TYP:CODE]` via l'`EditorHandle` — attaché au champ du panneau | M-03, M-04, M-11 |
| `content/display.js` | Calque de décoration cloisonné (shadow DOM) : légende, infobulle, soulignement — attaché au champ du panneau | M-05 |
...
actuelle (voir bugs.md pour l'historique complet des retours de test) :

1. Un adaptateur générique déduisant la zone de réponse par proximité du
   bouton d'envoi. Fonctionnait sur les fixtures, se trompait de zone dès
   qu'un vrai composer avait plusieurs boutons avant celui d'envoi.
2. Des adaptateurs dédiés (`chatgpt.js`, `claude.js`) à sélecteurs exacts.
   Toujours en échec après test réel.
3. Un scan de toute la page, déclenché par stabilité, pour la détection
   des champs de saisie **du site**. Résolvait la lecture (M-07) mais rien
   côté écriture : ProseMirror/Lexical n'acceptent une écriture
   programmatique que via `execCommand('insertText')` sur un champ
   explicitement identifié — deviner un composer par heuristique n'aide
   pas à résoudre ce problème.

Les trois généraient la même classe de problème : **deviner où agir sur le
site**. [ADR-008](adr/0008-side-panel.md) élimine le problème plutôt que de
le résoudre une quatrième fois : composition et lecture vivent dans le
panneau ; le site n'est atteint que sur un ciblage **explicite** (clic
droit) et persistant (descripteur par site, pas une détection à chaque
chargement). `site-adapters/` (matches/getInputFields/getResponseContainer/
isStreaming/onStreamingEnd) est supprimé — plus aucun composant ne cherche
à identifier automatiquement un champ ou une zone sur le site.
...
// fogbank.sites[]
{
  ...
  cibleEcriture: {                          // NOUVEAU (ADR-008) — descripteur best-effort, M-15 ; null si jamais ciblé
    id: string | null,
  ...
  configurationTerminee: boolean            // NOUVEAU (UC-005) — false tant que le parcours d'onboarding n'est pas passé une fois ; le panneau l'affiche tant que false, sans jamais bloquer la composition
}
...
1. Le panneau attache, à son unique champ de composition
   (`<textarea>` propre à fogbank), un `EditorHandle`
   (`TextareaHandle`), `mention-menu.js` et `display.js` — exactement le
   même câblage qu'un content script attachait auparavant à un champ
   détecté sur le site.
...
3. Sélection (M-04) → le tag `[TYP:CODE]` est inséré **directement dans le
   champ du panneau** via `EditorHandle.replaceRange`, `CODE` étant
   l'alias de l'entité pour le site **actuellement ciblé** (ou, à défaut,
   le site correspondant à l'onglet actif ; sinon `formatParDefaut`).
   Rotation paresseuse (voir plus bas) au même moment.
4. `display.js` détecte le tag fraîchement inséré et met à jour la légende
   + le soulignement, dans le panneau uniquement.
...
2. Le panneau envoie le texte du champ de composition au content script de
...
3. Le panneau résout chaque tag `[TYP:CODE]` complet trouvé via l'annuaire
...
- Pas de `chrome.alarms` ni de balayage périodique de tout l'annuaire :
  ...
  est effectivement utilisé**, lors de l'insertion du tag dans le panneau
  (Composition, étape 3) : comparaison de `expireLe` de l'`aliasParSite`
...
plutôt qu'a priori) mais ce n'est **pas encore implémenté**, faute de
spike dédié (S4 du plan initial) — voir « Travail restant ».
...
| M-08 | `sidepanel/sidepanel.js`, inline lors de l'insertion du tag (rotation paresseuse, pas de composant dédié) |
...
- **Permission à la demande** (S4, non spike-testé) : remplacer les
  `matches` larges des content scripts par `activeTab` + `contextMenus`,
  pour que fogbank ne s'exécute que sur un geste explicite plutôt que sur
  tout `<all_urls>` — voir § Permissions.
...
`popup/` (statut/toggle du site courant) sont implémentés. `site-adapters/`
et `content/reception.js` sont supprimés — plus de composant cherchant à
identifier automatiquement un champ ou une zone sur le site. Reste à faire :
voir § Travail restant.
